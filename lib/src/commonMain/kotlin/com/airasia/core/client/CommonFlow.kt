 ...
 * here instead of exposing a flow directly, they are changing the semantics silightlt.
 * So instead of flow, we only have callbacks that looks almost like a flow,
 * It also exposes a Closeable. that is responsbile for closing any scope we have on Kotlin side. so client can simple call close().
 ...
 * Threading controlled on Kotlin KMM. e.g incase the client want to run this flow in a Displather background it will still run on Main. but will explorewr this some other time.
 *
 * Added error handling
 *If the Flow is wrapped in a Task and using CoroutineExceptionHandler we can grab the throws witout crash
 * but what if the Flow is not using a wrapper, it crashes the app.
 * Here I added a way to cactch aby errors during the streaming of flows and send it as a callback
 ...
 * On Android we can srill use collect instead of watch
 */
